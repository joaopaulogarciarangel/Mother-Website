(function(a, n) {
  ...
  const n = {
    ...
    cA = (...t) => t.filter((h, d, c) => !!h && h.trim() !== "" && c.indexOf(h) === d).join(" ").trim(),
    MA = t => t.replace(/^([A-Z])|[\s-_]+(\w)/g, (h, d, c) => c ? c.toUpperCase() : d.toLowerCase()),
    pA = t => {
      const h = MA(t);
...
lucide.createIcons({icons});\``);
    ...
    if (Array.from(c.querySelectorAll(`[${h}]`)).forEach(p => za(p, {
        ...
        icons: t,
        attrs: d
      })), M && Array.from(c.querySelectorAll("template")).forEach(p => aA({
        icons: t,
        nameAttr: h,
        attrs: d,
        root: p.content,
